...
function(declare,
  ...
  return declare([BaseWidget], {
    ...
    updateScenario: function() {
      ...
      //get all scenarios except for current scenario
      aCompareScenarios = [];
      aCompareScenarios.push({"label" : 'No Comparison', "value" : 'none'});
      for (s in scenarios){
        if (scenarios[s].value!=sScenario) {
          aCompareScenarios.push({"label" : scenarios[s].label, "value" : scenarios[s].value});
        }
      }

      //setup comparison scenario combobox
      if (iFirst) {
        cmbScenarioCompare = new Select({
          options: aCompareScenarios,
          onChange: function() {
            sScenarioCompare = this.value;
            cW.updateComparison();
          }
        }, "cmbScenarioCompare");
        cmbScenarioCompare.startup();
        cmbScenarioCompare.set("value",'none');
        iFirst = false;
      } else {
        cmbScenarioCompare.set("options", aCompareScenarios).reset();
        cmbScenarioCompare.set("value", 'none');
        cmbScenarioCompare.startup();
      }
    ...
    updateComparison: function() {
      ...
      console.log('UpdateDistributionLayer begin');
      var query, updateFeature;
      var tmpDistrictID;
      var displayText;
      var blShareSelected = false;
              
      //See if a "Share" button is selected
      blShareSelected = this.IsShareSelected();
      ...
      //Read base layer values
      this.lyrSegments.queryFeatures(query,function(featureSet) {
        //Update values
        var resultCount = featureSet.features.length;
        for (var i = 0; i < resultCount; i++) {
          updateFeature = featureSet.features[i];
          tmpDistrictID = updateFeature.attributes[sAirSageIDField]
          for (var j = 0; j < aResults_Districts.length; j++) {
            if (tmpDistrictID == aResults_Districts[j]){
              if (blShareSelected) {
                updateFeature.attributes[sTripsPctField] = aResults_Trips_Share[j];
              } else {
                updateFeature.attributes[sTripsPctField] = aResults_Trips_Pct[j];
              }
    ...
    updateVCDisplay: function() {
      ...
      if (typeof seasons !== 'undefined' && typeof dowpeaks !== 'undefined') {
        ...
        dom.byId("NameCalculationDetails").innerHTML = scenarios.find(o => o.value === sScenario).label + ' - ' + seasons.find(o => o.value === sSeason).label + ' ' + dowpeaks.find(o => o.value === sDOWPeak).label
      } else {
    ...
    updateSegmentDetails: function() {
      ...
      dojo.xhrGet({
        url: "widgets/CongestionSidebar/data/segment_vcdata/" + sScenario + "/" + sScenario + "_" + g_sSegID + ".json",
        handleAs: "json",
        load: function(obj) {
          /* here, obj will already be a JS object deserialized from the JSON response */
          console.log(sScenario + "_" + g_sSegID + ".json");
          segment_vcdata = obj;

          //get vcdata for a given segment
          let segdata  = segment_vcdata.find(segment_vcdata => segment_vcdata.SegID = g_sSegID);
          
          //segment and forecast
          dom.byId("SegmentID").innerHTML = segdata.SegID;
          dom.byId("Forecast").innerHTML = segdata.ForecastAdj;
    ...
    toggleCompareScenario: function() {
      var divCompare = dom.byId("comparescenario");

      if (divCompare.style.display == sBlockShow) {
        divCompare.style.display = sBlockHide;
      } else {
        divCompare.style.display = sBlockShow;
      }
    },
